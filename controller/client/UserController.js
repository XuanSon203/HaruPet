...
module.exports.loginPost = async (req, res) => {
  try {
    ...
    if (password !== user.password) {
      ...
      // Nếu số lần sai vượt quá giới hạn
      if (user.loginAttempts >= MAX_LOGIN_ATTEMPTS) {
        user.status = "inactive"; // khóa tài khoản
        await user.save();
        req.flash("error", "Bạn đã nhập sai quá nhiều lần. Tài khoản đã bị khóa.");
        return res.redirect("/user/login");
      }

      await user.save(); // lưu lại số lần sai
      req.flash("error", `Sai mật khẩu (${user.loginAttempts}/${MAX_LOGIN_ATTEMPTS})`);
      return res.redirect("/user/login");
    }

    // Nếu đăng nhập đúng ⇒ reset loginAttempts về 0
